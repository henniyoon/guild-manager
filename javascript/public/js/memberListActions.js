async function goToGuildSetting() {
  window.location.href = '/guildSetting';
}

async function goToMemberSync() {
   // 서버에 업데이트 요청을 보내는 코드
   const response = await fetch(`/memberSync`, {
    method: 'POST',
  });

  // 서버에서 온 응답을 확인하고 필요한 작업을 수행
  if (response.ok) {
    console.log('길드원 동기화 성공');
    
    // 리다이렉트 후에 새로고침
    window.location.reload();
  } else {
    console.error('길드원 동기화 실패');
  }
}

async function toggleEditable(cell, subMemberName) {
